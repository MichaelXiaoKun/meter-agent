...
function validationStartTitle(mode: unknown): string {
  const normalized = narrowStr(mode).toLowerCase();
  if (normalized === "rough" || normalized === "skipped") return "Preparing answer";
  if (normalized === "strong") return "Validating product claims";
  return "Validating the answer";
}
...
export function reduceTurnActivity(
  ...
  switch (event.type) {
    ...
    case "validation_start":
      return push({
        kind: "validation",
        title: validationStartTitle(event.validation_mode),
