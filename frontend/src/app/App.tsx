...
import { getAuthViewFromHash, setHashForAuthView, type AuthGateView } from "../features/auth/authGate";
import { clearAuth, getStoredAuth, setAuth } from "../features/auth/authStorage";
import LoginPage from "../features/auth/components/LoginPage";
import ForgotPasswordPage from "../features/auth/components/ForgotPasswordPage";
import CheckMailPage from "../features/auth/components/CheckMailPage";
import EntryChoicePage from "../features/auth/components/EntryChoicePage";
import SalesChatPage from "../features/sales/SalesChatPage";
import Sidebar from "../features/conversations/components/Sidebar";
import SidebarIconRail from "../features/conversations/components/SidebarIconRail";
import ChatView from "../features/chat/components/ChatView";
import { ToastContainer } from "../features/feedback/components/Toast";
import { useToast } from "../features/feedback/useToast";
import { readStoredModel, writeStoredModel } from "../features/chat/components/modelPickerStorage";
...
export default function App() {
  ...
  const adminSendInFlightRef = useRef(false);
  ...
  async function handleSend(
    ...
    adminSendInFlightRef.current = true;
    try {
      ...
      toast.error(
        "Can’t reach FlowIQ",
        err instanceof Error ? err.message : "The API server is unavailable.",
      );
    } finally {
      adminSendInFlightRef.current = false;
    }
  }
  ...
  const sidebarProps = {
    ...
    conversationListState: conversationsError
      ? {
          kind: "error" as const,
          message: "Reconnecting to API server...",
        }
      : conversationsLoading && !listLoaded
        ? {
      ...
      */}
      ...
      <main className="flex min-h-0 min-w-0 flex-1 flex-col">
        <ChatView
          ...
          disabled={!!conversationsError}
          disabledMessage={
            conversationsError
              ? "Reconnecting to the API server. Keep this tab open; chats will return automatically."
              : undefined
          }
