...
import { formatConversationRelativeDate, getDefaultDateLocale } from "../../../core/conversationRelativeDate";
import {
  buildConversationListItems,
  isConversationUnread,
  sectionLabel,
  type ListItem,
} from "../../../core/conversationListModel";
...
export default function ConversationList({
  ...
  const processingSet = useMemo(
    () => new Set([...(processingId ? [processingId] : []), ...processingIds]),
        ...
        {emptyState ? (
          <div className="mx-auto flex max-w-[14rem] flex-col items-center gap-2">
            {emptyState.kind === "loading" && (
              <span
                className="h-4 w-4 animate-spin rounded-full border-2 border-brand-border border-t-brand-500"
                aria-hidden
              />
            )}
          ...
          return (
            <div key={it.id} style={pos} className="box-border h-full min-h-0 min-w-0 px-0">
              <ConversationRow
                ...
                isBusy={processingSet.has(it.conv.id)}
