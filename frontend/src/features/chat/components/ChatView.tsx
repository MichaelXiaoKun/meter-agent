...
import { extractDownloadArtifacts } from "../../../core/artifactAttachments";
import { PlotGrouped } from "./PlotImage";
import ArtifactLinks from "./ArtifactLinks";
import WelcomeCard from "./WelcomeCard";
import type { QuickAction } from "./WelcomeCard";
import WelcomeBluebotLogo from "../../branding/components/WelcomeBluebotLogo";
...
} from "../../../core/turnActivity";
import { useMediaQuery } from "../../../hooks/useMediaQuery";
import { useSpeechRecognition } from "../../../hooks/useSpeechRecognition";
import { buildMeterWorkspace } from "../../../core/meterWorkspace";
import MeterWorkspacePanel from "../../meter-workspace/components/MeterWorkspacePanel";
import {
  configAngle,
  configSerial,
  configSweepAngles,
  configSweepRangeLabel,
} from "../../../core/configCompat";
...
export default function ChatView({
  ...
  const submitGuardRef = useRef<{ text: string; until: number } | null>(null);
  ...
  function handleSubmit(text?: string) {
    ...
    const now =
      typeof performance !== "undefined" && typeof performance.now === "function"
        ? performance.now()
        : Date.now();
    const guarded = submitGuardRef.current;
    if (guarded && guarded.text === trimmed && guarded.until > now) return;
    submitGuardRef.current = { text: trimmed, until: now + 1000 };
    if (submitGuardTimerRef.current != null) {
      window.clearTimeout(submitGuardTimerRef.current);
    }
  ...
  const transcriptSwitchKey = conversationId ?? "welcome";
  ...
  const composerDisabledMessage =
    disabledMessage ?? "Enter your bluebot token in Settings to start chatting.";
              ...
              <div className="flex shrink-0 items-center gap-2">
                ...
                {showHeaderLogo && (
                  <motion.div
                    ...
                    animate={{ opacity: 1, scale: 1 }}
                    transition={CHAT_SWITCH_TRANSITION}
                  >
                    <WelcomeBluebotLogo
                      size={32}
                      mood={headerLogoMood}
