...
def analyze(df: pd.DataFrame, serial_number: str, verified_facts: Optional[Dict[str, Any]] = None) -> str:
    ...
    system_prompt = (
        ...
        "`verified_facts_precomputed` may include `diurnal_seasonality`, which compares current local-hour medians "
        "against a reference hour-of-day profile; use its `departure_score` and per-hour z-scores only when "
        "`diurnal_seasonality.state=scored`, otherwise relay its refusal state without inventing seasonality claims. "
        "`verified_facts_precomputed` may include `threshold_events` for user-requested threshold predicates. "
        "Use event counts and event windows only when `threshold_events.state=ready`; if it is "
        "`invalid_predicate` or `not_evaluated`, relay `reasons_refused` verbatim instead of inventing events. "
        "`verified_facts_precomputed` may include `frequency_domain` with dominant spectral periods; use it only "
        "when `frequency_domain.state=ready` and do not infer periodicity if it is absent. "
