...
# ---------------------------------------------------------------------------
# Native admin tickets / work items
# ---------------------------------------------------------------------------
...
def create_ticket(body: CreateTicketRequest, authorization: str = Header(...)):
    _bearer_token(authorization)
    try:
...
def create_ticket_event(
    ticket_id: str,
    body: CreateTicketEventRequest,
    authorization: str = Header(...),
):
    _bearer_token(authorization)
    try:
        return store.append_ticket_event(
            ticket_id=ticket_id,
            user_id=body.user_id,
...
@app.post("/api/public/sales/conversations")
def create_sales_conversation(body: SalesConversationRequest | None = None):
    """Create a public, pre-login sales conversation."""
    title = (body.title if body else "") or "Sales conversation"
    conv_id = store.create_sales_conversation(title[:80])
    return {"id": conv_id}


@app.get("/api/public/sales/conversations")
def list_sales_conversations(ids: str = Query(default="")):
    ...
    return store.list_sales_conversations(wanted[:100])


@app.get("/api/public/sales/conversations/{conv_id}")
def get_sales_conversation(conv_id: str):
    """Load a public sales conversation without Auth0."""
    if not store.sales_conversation_exists(conv_id):
        raise HTTPException(404, "Sales conversation not found")
    return {
        "id": conv_id,
        "messages": store.load_sales_messages(conv_id),
        "lead_summary": store.load_sales_lead_summary(conv_id),
    }


@app.get("/api/public/sales/conversations/{conv_id}/status")
def sales_conversation_status(conv_id: str):
    ...
    with _streams_lock:
        stream_id = _active_conversation_streams.get(active_key)
        ...
        processing = active_key in _active_conversations or (
...
@app.patch("/api/public/sales/conversations/{conv_id}")
def patch_sales_conversation(conv_id: str, body: UpdateTitleRequest):
    """Rename a public sales conversation."""
    if not store.sales_conversation_exists(conv_id):
        raise HTTPException(404, "Sales conversation not found")
    store.set_sales_title(conv_id, body.title[:120])
    return {"ok": True}


@app.delete("/api/public/sales/conversations/{conv_id}")
def delete_sales_conversation(conv_id: str):
    """Delete a public sales conversation."""
    if not store.sales_conversation_exists(conv_id):
        raise HTTPException(404, "Sales conversation not found")
    store.delete_sales_conversation(conv_id)
    return {"ok": True}


@app.post("/api/public/sales/conversations/{conv_id}/share")
def create_sales_conversation_share(conv_id: str):
    """Create a public read-only snapshot link for a sales conversation."""
    try:
        token, revoke_key = store.create_sales_share(conv_id)
    except LookupError as e:
...
@app.delete("/api/public/sales/shares/{token}")
def delete_sales_share(token: str, revoke_key: str = Query(...)):
    """Revoke a public sales share using the browser-held revoke key."""
    ok = store.revoke_sales_share(token, revoke_key)
    if not ok:
        raise HTTPException(404, "Share not found or access denied")
    return {"ok": True}


@app.post("/api/public/sales/conversations/{conv_id}/cancel")
...
async def sales_chat_init(conv_id: str, body: SalesChatRequest):
    ...
    store.append_sales_messages(conv_id, [user_msg])
    if checkpoint == 0:
        store.set_sales_title(conv_id, body.message[:60])
    ...
    with _streams_lock:
        ...
        _active_conversations.add(active_key)
        _active_conversation_streams[active_key] = stream_id
        _cancel_events[active_key] = cancel_event
    ...
    def _run() -> None:
        ...
        try:
            ...
            with _streams_lock:
                _cancelled_conversations.discard(active_key)
                _cancel_events.pop(active_key, None)
                _active_conversations.discard(active_key)
                if _active_conversation_streams.get(active_key) == stream_id:
                    _active_conversation_streams.pop(active_key, None)
            if slot_acquired:
...
@app.get("/api/public/sales/streams/{stream_id}")
...
@app.get("/api/public/sales/streams/{stream_id}/poll")
